...
TZ=os.getenv('TIMEZONE','America/New_York'); AUTO=os.getenv('AUTO_SEND','false').lower()=='true'; TH=float(os.getenv('MIN_MATCH_SCORE','0.86'))
...
def digest():
    rows=open_followups()
    if not rows: return
    lines=['*2:30 PM Follow-up List*','These items still need your attention today:','']
